def lowercasedata(datafr):
    datafr=datafr.apply(lambda x: x.astype(str).str.lower())
    return datafr

def listconv(datafr):
    datafr.skills=[skill.split("\n") for skill in DS_jobs_df.skills]
    datafr.locations=[location.split(",") for location in DS_jobs_df.locations]
    return datafr
